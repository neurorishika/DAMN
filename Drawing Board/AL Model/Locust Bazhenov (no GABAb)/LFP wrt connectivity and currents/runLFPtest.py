...
y = np.where(t<1200,np.ones(t.shape),np.zeros(t.shape))
y = np.where(t<200,np.zeros(t.shape),y)

# Randomly choose indices from PNs

p_input = float(sys.argv[1])
input_neurons = np.random.choice(np.array(range(p_n)),int(p_input*p_n),replace=False)

pn_input = int(sys.argv[2])
ln_input = int(sys.argv[3])

# Assign input shape to chosen indices
for i in input_neurons:
    current_input[i,:]= pn_input*y
for i in range(90,120):
    current_input[i,:]= ln_input*y
...
np.save("p_{}_pn_{}_ln_{}".format(sys.argv[1],sys.argv[2],sys.argv[3]),state[:,:120])
