...
def generate_orn(duration,resolution,odorVec,odorStart,odorEnd): # Function to generate single ORN Trace
    ...
    rec_field = np.random.uniform(-1,1,l_odorspace) # Receptive Field of ORNs in Odor Space
    ...
    if np.arccos(cosSim) < np.pi*82/180:	# Minimum Response Threshhold
        res_strength = (1-baseline)*sigmoid(odorMag*np.cos(np.arccos(cosSim))**tuning)
    else:
...
for i in range(100):	# Generate 100 ORN types
    time,trace = generate_orn(6000,0.01,odor,500,1500)
    orns.append(trace)
