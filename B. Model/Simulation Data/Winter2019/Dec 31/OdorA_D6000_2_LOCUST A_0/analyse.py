...
plt.scatter(events[:,1],events[:,0],marker='s',s=1.0,c='r')
plt.xlim([0,6000])
plt.yticks([])
plt.tight_layout()
plt.box(False)
plt.savefig('Raster_LNs.png', transparent=True)
...
plt.tight_layout()
plt.box(False)
plt.savefig('Raster_PNs.png', transparent=True)
...
ax.spines['top'].set_visible(False)   
ax.spines['bottom'].set_visible(False)
plt.plot(runningResponsive(lol,100)*100,c='r')
plt.tight_layout()
plt.ylim([0,30])
plt.savefig('Res.png', transparent=True)
